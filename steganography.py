...
class Ui_Dialog(object):
    def setupUi(self, Dialog):
        ...
        Dialog.resize(348, 105)
        self.progressBar = QtWidgets.QProgressBar(Dialog)
        self.progressBar.setGeometry(QtCore.QRect(30, 30, 291, 21))
...
class Steganography(object):
    ...
    def merge(img1, img2, filename):
        ...
        filename = 'C:\\Users\\Sridhara\\Desktop\\' + filename
        new_image.save(filename)
        Dialog.close()
        return new_image

    @staticmethod
    def unmerge(img, filename):
        """Unmerge an image.
...
def merge(img1, img2, filename):
    t1 = threading.Thread(target=Steganography.merge, args=(Image.open(img1), Image.open(img2), filename))
    t1.start()
    Dialog.show()


def unmerge(img, filename):
    t2 = threading.Thread(target=Steganography.unmerge, args=(Image.open(img), filename))
    t2.start()
    Dialog.show()
